...
const CODE_COMPLETION_SYSTEM_PROMPT = `
You are an AI inline code completion engine similar to GitHub Copilot.

Your output is inserted directly into the user's source code at the cursor.

Rules:
- Return ONLY the text that should be inserted.
- Never use Markdown or code fences.
- Never explain your answer.
- Never repeat code already before the cursor.
- Never rewrite or modify existing code.
- Never repeat code already after the cursor.
- Preserve indentation, formatting, quotes, semicolons, and coding style.
- Generate the smallest useful completion.
- Match the project's language, framework, and conventions.
- Complete only what naturally comes next.
- Stop immediately once the completion is finished.
- If the intent is unclear, return an empty response.
`;

const copilot = new CompletionCopilot(undefined, {
  model: async ({ context, instruction, fileContent }) => {
    const prompt = `
...
`;
    ...
    const response = await fetch(
      process.env.OLLAMA_URL ?? "http://127.0.0.1:11434/api/generate",
      {
        ...
        body: JSON.stringify({
          model: process.env.OLLAMA_CODE_MODEL ?? "qwen2.5-coder:3b",
          prompt,
          stream: false,
          options: {
            temperature: 0.1,
    ...
    return {
      text: removeCodeFence(result.response ?? "") || null,
    };
  },
});
